...
# ComfyUI integration for progress bar and interrupt support
try:
...
class DreamIDV:
    ...
    def load_image_latent_ref_ip_video(self,paths: str, size, device, frame_num):
        ...
        for i, path in enumerate(paths):
            frames = []
            is_tensor = isinstance(path, torch.Tensor)
            input_type = "tensor" if is_tensor else is_image_or_video_by_extension(path)
            
            # 1. Load Frames into generic PIL format
            if input_type == "tensor":
                ...
                arr = (t.cpu().float().numpy() * 255).clip(0, 255).astype('uint8')
                for frame in arr:
                    frames.append(Image.fromarray(frame))
                    
            elif input_type == "video":
                if i == 0: print('ori_path', path)
                vr = VideoReader(path)
                for idx in range(len(vr)):
                 ...
                 # For image, we handle single frame
                 with Image.open(path) as img:
                      img = img.convert("RGB")
                      frames.append(img)
            
            else:
                print(f"Unknown file type or index: {i}")
                continue

            if not frames:
                continue

            # 2. Set/Get Video Dimensions from Reference Video (i=0)
            if i == 0:
                video_w, video_h = frames[0].size
                
            # 3. Process Logic based on Index
            ...
            if i == 0:
                ...
                video_transform=Compose([
                    NaResize(resolution=math.sqrt(size[0] * size[1]), downsample_only=True),
            ...
            # --- Reference Image (i=2) ---
                ...
                # Image Padding and Resize Logic
                img = frames[0] # Use loaded PIL image
            ...
            # --- Pose Video (i=3) ---
                ...
                video_frames = pose_transform(frames) 
    ...
    def generate(self,
        ...
        with amp.autocast(dtype=self.param_dtype), torch.no_grad(), no_sync():
            ...
            # Create ComfyUI progress bar if available
            pbar = ProgressBar(len(timesteps)) if HAS_COMFY else None
            
            # Create console progress bar (always)
            from tqdm import tqdm
            pbar_console = tqdm(total=len(timesteps), desc="DreamID-V Sampling")
            ...
            # Return latent or decoded video based on parameter
            if return_latent:
                # Return raw latent for external VAE decode
                result = x0[0]  # [C, T, H, W]
            else:
                if self.rank == 0:
        ...
        if offload_video_model or offload_t5:
            gc.collect()
